...
const WeightLoss =({navigation}) =>{
    ...
    const handleSubmit=async()=>{
        try {
            const response = await fetch('http://X.X.X.X:5050/weightLoss', { //X.X.X.X Replaced by User Ethernet IP
              method: 'POST',
              headers: {
    ...
    return(
        <View style={{flex:1}}>
            ...
            <Text style={{fontSize:20,color:'white',textAlign:'center',backgroundColor: 'black',borderRadius: 10,overflow:'hidden',marginLeft:20,top:20,marginRight:20}}>Weight Loss</Text>
